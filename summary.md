...
- **Gemini 工具调用（Tool Calling）**：
  ...
  - `stream/interceptors.py` 拦截并解析模型输出的 JSON 格式工具调用指令（`{"tool_call": {...}}`），采用“检测→缓冲→保活”三阶段策略：跨 chunk 聚合 ```json``` 代码块、每 0.5 秒发出保活提示、2 秒超时兜底，确保 VSCode/客户端不会卡住或泄露原始 JSON
...
- **脚本化运维**：`scripts/start.sh`/`scripts/stop.sh` 提供一键进程管理，启动脚本会在写入 PID 前自动使用 `lsof` 清理占用 `2048/3120/9222` 的遗留 Camoufox/stream 进程（必要时强制 SIGKILL），停止脚本则基于 PID 安全下线进程，避免同一服务器多次部署后端口被卡住。
