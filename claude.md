...
interceptors.py
    ...
    ├── 方案 C 内容提取 → resp["body"] / resp["reason"]
    ├── Thinking 识别 → 英文内容放入 resp["reason"]
    ├── tool_call 识别 → resp["function"]
    └── 流式缓冲（第三版）→ 隐藏 ```json 块
    ↓
response_generators.py (SSE 生成)
    ├── reason → reasoning_content (VSCode 折叠显示)
    └── body → content (正常显示)
    ↓
客户端
```
...
## Thinking 内容识别与折叠显示（2025-12-03 晚）

### 问题描述

**现象**：Gemini 的 thinking（思考过程）内容在 VSCode 中直接以纯文本显示，而非以可折叠的形式展示。

**对比**：
- ❌ **问题状态**：thinking 以普通文本直接显示，占据大量空间
- ✅ **期望状态**：thinking 应该像 Claude/o1 模型一样，以可折叠的形式显示

### 根本原因

**数据路由错误**：thinking 内容被错误地放入了 `body` 字段，而非 `reason` 字段。

#### 错误的数据流：
```
Gemini thinking: "**Evaluating...**\n\n..."
    ↓
interceptors.py
    resp["body"] += content  ❌ 错误路由
    ↓
response_generators.py (第113-155行)
    delta["content"] = body  ❌ 通过 content 字段发送
    ↓
VSCode 接收 content
    显示为普通文本 ❌ 无折叠
```
...
Gemini thinking: "**Evaluating...**\n\n..."
    ↓
interceptors.py
    resp["reason"] += content  ✅ 正确路由
    ↓
response_generators.py (第91-111行)
    delta["reasoning_content"] = reason  ✅ 通过 reasoning_content 字段发送
    ↓
VSCode 接收 reasoning_content
    自动折叠显示 ✅ 像 o1 模型一样
```

### 关键发现

1. **系统已有 reasoning_content 支持**
   - `api_utils/response_generators.py` 第 91-111 行已实现 `reasoning_content` 字段
   - 这是 OpenAI o1 模型引入的标准字段
   - VSCode 能识别该字段并自动以折叠形式显示

2. **Thinking 内容特征**（通过分析原始响应数据发现）
   ```
   内容块 1: **Identifying the Active Environment**\n\n[英文思考内容]
   内容块 2: **Determining Best Approach**\n\n[英文思考内容]
   内容块 3: **Verifying the Active Env**\n\n[英文思考内容]
   内容块 4: **Analyzing Further Details**\n\n[英文思考内容]
   --- thinking 结束 ---
   内容块 5: 我
   内容块 6: 将在终端中运行...（中文回复）
   ```
...
3. **识别规则**
   - ✅ Thinking 总是位于响应的起始部分
   - ✅ Thinking 内容全部为英文
   - ✅ Thinking 以 `**标题**` 格式开头（markdown 加粗）
   - ✅ **关键判断**：出现中文字符时，thinking 阶段结束

### 解决方案实现

#### 1. 添加 thinking 阶段标志

**位置**：[stream/interceptors.py:30-31](stream/interceptors.py#L30-L31)
...
**位置**：[stream/interceptors.py:198-216](stream/interceptors.py#L198-L216)
...
**算法核心**：
1. 默认处于 thinking 阶段（`_in_thinking_phase = True`）
2. 持续将英文内容放入 `resp["reason"]`
3. 一旦检测到中文字符（`[\u4e00-\u9fff]`），标记 thinking 结束
4. 之后所有内容都放入 `resp["body"]`

#### 3. 重置 thinking 标志

**位置**：[stream/interceptors.py:424-425](stream/interceptors.py#L424-L425)
...
在每次响应结束时（`_reset_buffer_state`），重置标志为 `True`，准备处理下一个请求。
...
Gemini API 返回:
  ...
  Chunk 4: "我将在终端中运行..." (中文)
    ↓
interceptors.py (parse_response):
  Chunk 1-3: 检测英文 → resp["reason"] += content
  Chunk 4: 检测中文 → self._in_thinking_phase = False
           → resp["body"] += content
    ↓
数据结构:
  {
    ...
    ↓
response_generators.py (gen_sse_from_aux_stream):
  第 91-111 行: reason → reasoning_content
  第 113-155 行: body → content
    ↓
SSE 输出:
  data: {"delta": {"reasoning_content": "**Identifying...**\n\n..."}}
  data: {"delta": {"content": "我将在终端中运行..."}}
    ↓
VSCode 接收:
  reasoning_content → 自动折叠显示 ✅
  content → 正常显示 ✅
```
...
- ✅ **简单可靠**：基于语言特征（中文 vs 英文）判断
- ✅ **零配置**：利用现有 `reasoning_content` 基础设施
- ✅ **自动适配**：VSCode 原生支持，无需额外客户端修改
- ✅ **性能开销小**：只需一次正则匹配 `[\u4e00-\u9fff]`

#### 限制

- ⚠️ **语言依赖**：假设 thinking 是英文，回复是中文
  - **适用场景**：中文用户使用 Gemini（最常见场景）
  - **不适用场景**：英文用户使用 Gemini（thinking 和回复都是英文）

- 🔄 **可能的改进**：
  - 检测连续的 `**标题**` 模式来识别 thinking
  - 添加语言检测库（如 `langdetect`）进行更精确判断
  - 在提示词中明确要求 Gemini 用特定标记分隔 thinking 和回复
...
3. **触发请求**
   - 在 VSCode 中向 Gemini 模型发送请求
   - 观察响应中的 thinking 部分

4. **预期结果**
   - ✅ Thinking 内容应该显示为可折叠的块
   - ✅ 类似 Claude/o1 模型的展示方式
   - ✅ 普通回复内容正常显示
...
检查日志中的提取信息：
```
[提取] ✓ Thinking 内容: 420 字符
[提取] ✓ Thinking 内容: 641 字符
[提取] ✓ 普通内容（thinking 已结束）: 43 字符
```
...
| 文件 | 修改内容 | 行号 |
|------|---------|-----|
| `stream/interceptors.py` | 添加 `_in_thinking_phase` 标志 | 30-31 |
| `stream/interceptors.py` | 实现 thinking 识别逻辑 | 198-216 |
| `stream/interceptors.py` | 重置 thinking 标志 | 424-425 |
| `api_utils/response_generators.py` | 已有 `reasoning_content` 支持（无需修改） | 91-111 |

### 后续优化方向

如果当前基于中文检测的方案不够准确，可以考虑：
...
2. **方案 B：提示词优化**
   - 在系统提示词中要求 Gemini 用特殊标记分隔
   - 例如：`<!-- THINKING END -->`
   - 优点：最可靠
   - 缺点：需要修改提示词

3. **方案 C：语言检测库**
   - 使用 `langdetect` 或 `fasttext` 检测语言
   - 优点：更精确
   - 缺点：增加依赖

---

**文档最后更新**: 2025-12-03 17:30
**当前版本**: 第三版流式缓冲 + 方案 C 内容提取 + Thinking 折叠显示
**状态**: ⏳ 已实现，待测试验证
